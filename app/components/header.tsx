        ...
        >
          ...
          <ul className="hidden lg:flex items-center gap-8 font-semibold">
            <NavItem
              ...
              shadowClass="group-hover:drop-shadow-[0_0_10px_rgba(59,130,246,0.5)]"
            />
            ...
            {/* Optimized More Dropdown */}
            <li
              className="relative cursor-pointer group list-none"
              onMouseEnter={() => setOpen(true)}
              onMouseLeave={() => setOpen(false)}
            >
              <div className="h-7 overflow-hidden transition-all duration-300 group-hover:drop-shadow-[0_0_10px_rgba(249,115,22,0.6)]">
                <div className="relative transition-transform duration-500 ease-out group-hover:-translate-y-7">
              ...
              {/* Dropdown Menu */}
              {open && (
                <div className="absolute top-8 left-0 bg-white rounded-2xl  p-3 w-60 space-y-1 border border-gray-300 animate-in fade-in slide-in-from-top-2 duration-300">
                  {["About", "eSIM Compatible Phones", "FAQ", "Blog"].map(
                    (item) => (
                      <div
                        key={item}
                        className="px-4 py-2 text-xs text-black hover:bg-orange-50 hover:text-orange-600 rounded-xl cursor-pointer transition-all font-bold"
                      >
                        {item}
                      </div>
                    ),
